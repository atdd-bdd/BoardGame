...
def before_scenario(context, scenario):
    ...
    # My dummy attacks: rows 1-4, cols E-J  (safe — default opp ships are at rows 6-9)
    context.dummy_my_locs  = [f'{chr(ord("A")+c-1)}{r}' for r in range(1, 5)  for c in range(5, 11)]
    # Opp dummy attacks: rows 5-10 (safe — test ship positions stay in rows 1-4)
    context.dummy_opp_locs = [f'{chr(ord("A")+c-1)}{r}' for r in range(5, 11) for c in range(1, 11)]
